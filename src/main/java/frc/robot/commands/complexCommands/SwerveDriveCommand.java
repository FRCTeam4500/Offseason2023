 ...
 * When pushing the joystick forward, the robot moves down the field, no matter
 * which way it is facing (Actually, it moves in whatever direction is zeroed
 * to, this just assumes that the gyro is zeroed down the field) <p>
 * Robot-Centric:
 * The robot moves relative to itself.
 * When pushing the joystick foward, the robot moves in whatever direction it is
 * facing. For our purposes, the front of the robot is the intake side.
 */
public class SwerveDriveCommand extends CommandBase {
  ...
  public SwerveDriveCommand(DriveController xbox) {
    ...
    controller = xbox;
    controlMode =
        ControlMode.FieldCentric; // default control mode is field-centric
    angleController = new PIDController(1, 0, 0);
    angleController.enableContinuousInput(-Math.PI, Math.PI);
    fastSpeed();
    addRequirements(swerve);
    Shuffleboard.getTab("Display").addString("Drive Mode",
                                             () -> controlMode.name());
  }
  ...
  private void moveAngleCentric(double xSpeed, double ySpeed) {
    double wSpeed =
        3.75 * angleController.calculate(swerve.getRobotAngle(),
  ...
  public void switchControlMode() {
    if (controlMode == ControlMode.FieldCentric) {
      ...
      MessagingSystem.getInstance().addMessage(
          "Swerve -> Control Mode -> Robot Centric");
    } else {
      controlMode = ControlMode.FieldCentric;
      MessagingSystem.getInstance().addMessage(
          "Swerve -> Control Mode -> Field Centric");
    }
  }
