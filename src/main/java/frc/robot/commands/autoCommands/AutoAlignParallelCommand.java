...
public class AutoAlignParallelCommand extends CommandBase {
  ...
  public void initialize() {
    ...
    if (!vision.hasValidTargets(0)) {
      MessagingSystem.getInstance().addMessage("No valid targets for " +
                                               getName());
      CommandScheduler.getInstance().schedule(new RumbleCommand(0.5));
      CommandScheduler.getInstance().cancel(this);
    }
    rotationOffset = vision.getRelativeTargetPose(0).getRotation().getDegrees();
  }

  @Override
  public void execute() {
    rotationOffset = pid.calculate(
