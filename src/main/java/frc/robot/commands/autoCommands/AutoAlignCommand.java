...
public class AutoAlignCommand extends SequentialCommandGroup {
  ...
  public AutoAlignCommand(VisionTarget target) {
    ...
    switch (target) {
    case GamePiece:
      addCommands(new AutoAlignRotationalCommand(target),
                  new AutoDriveToCommand(target));
      break;
    case AprilTag:
      Vision.getInstance().setPipeline(target.limelightId, target.pipeline);
      ...
      addCommands();
    case ReflectiveTape:
      addCommands(new AutoTurnCommand(targetAngle),
