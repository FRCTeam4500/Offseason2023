...
public class AutonomousDrivingCommand extends CommandBase {
  ...
  public AutonomousDrivingCommand(AutoDriveStage stage) {
    ...
    speedMultiplier =
        (stage.getMaxSpeed() / SwerveConstants.MAX_LINEAR_SPEED) % 1.0;
    forwardPID.reset();
    sidewaysPID.reset();
    rotationalPID.reset();
  ...
  public void execute() {
    ...
    case RelativePoseAlign:
      targetRelativePose = stage.offsetPose.relativeTo(
          swerve.getRobotPose().relativeTo(stage.originPose));
      forwardVel = speedMultiplier *
                   Math.abs(forwardPID.calculate(targetRelativePose.getX()));
      sidewaysVel = speedMultiplier *
                    Math.abs(sidewaysPID.calculate(targetRelativePose.getY()));
      rotationalVel =
          speedMultiplier * Math.abs(rotationalPID.calculate(
                                targetRelativePose.getRotation().getRadians()));
      swerve.driveModules(
          new ChassisSpeeds(forwardVel, sidewaysVel, rotationalVel));
      if (HelperMethods.isClose(targetRelativePose, new Pose2d(), 0.25,
                                Math.PI / 4)) {
        timesCorrect++;
      } else {
        timesCorrect = 0;
  ...
  public boolean isFinished() { return timesCorrect > 50; }
  ...
  public void initSendable(SendableBuilder builder) {
    ...
    builder.addDoubleProperty("Target X",
                              () -> targetRelativePose.getX(), null);
    builder.addDoubleProperty("Target Y",
                              () -> targetRelativePose.getY(), null);
    builder.addDoubleProperty(
        "Target Z", () -> targetRelativePose.getRotation().getDegrees(), null);
  }
  ...
  public static class AutoDriveStage {
    ...
    public AutoDriveStage(AutoDriveMode driveMode, Pose2d offsetPose,
                          double maxSpeed) {
