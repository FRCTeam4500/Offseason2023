...
public class TalonMotorController
    extends BaseTalon implements SwerveMotorController {
  ...
  public TalonMotorController(int deviceID, TalonType type) {
    ...
    default:
      MessagingSystem.getInstance().addMessage(
          "Can ID #" + deviceID + "'s motor model isn't a valid option");
    }
  }
  ...
  public double getOutput() { return getMotorOutputPercent(); }
  ...
  public void configureForSwerve(boolean isInverted, int currentLimit,
                                 double kP, double kD, boolean isDriveMotor) {
    if (isDriveMotor) {
      configSupplyCurrentLimit(new SupplyCurrentLimitConfiguration(
                                   true, currentLimit, currentLimit + 1, 0.1),
                               50);
      config_kP(0, kP);
      config_kF(0, 0.047);
