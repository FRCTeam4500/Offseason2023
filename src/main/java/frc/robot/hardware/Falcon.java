...
/**
 * Quick and dirty wrapper for TalonFX that should get us through the season. Stolen kindly form Highlander.
 */
public class Falcon extends TalonFX {
	...
	public Falcon(int id) {
		super(id);
		bandWithLimitMotorCAN(this);
	}

	public Falcon(int id, double gearing) {
		super(id);
		gearRatio = gearing;
		bandWithLimitMotorCAN(this);
	}

	public Falcon(int id, String canbus, double gearing) {
		super(id, canbus);
		gearRatio = gearing;
		bandWithLimitMotorCAN(this);
	}

	public Falcon(int id, String canbus) {
		super(id, canbus);
		bandWithLimitMotorCAN(this);
	}

	/** Makes a new Falcon with a PID controller built in */
	public Falcon(int id, double gearing, double p, double i, double d) {
		super(id);
		gearRatio = gearing;
		this.config_kP(0, p);
		this.config_kI(0, i);
	...
	public Falcon(
		...
		this.config_kI(0, i);
		this.config_kD(0, d);
		this.configMotionAcceleration(Falcon.rotToNative(accelRpmSquared));
		this.configMotionCruiseVelocity(Falcon.rpmToNative(maxVelRpm));
		bandWithLimitMotorCAN(this);
	}
	...
	public void defaultConfigs() {
		...
		this.configSupplyCurrentLimit(
				new SupplyCurrentLimitConfiguration(true, 40, 80, 0.5)
			);
	}
	...
	public double getRotations() {
		return (
			getSelectedSensorPosition() * TICKS_TO_ROTATIONS * (1.0 / gearRatio)
		);
	}
	...
	public void setTargetRPM(double rpm) {
		set(
			...
			rpm * ROTATIONS_TO_TICKS * 60 * 10 * gearRatio
		);
	}
	...
	public void setTargetRot(double rotations) {
		set(
			...
			rotations * ROTATIONS_TO_TICKS * gearRatio
		);
	}
	...
	/** Stolen kindly from 841. Limits CAN network usage */
