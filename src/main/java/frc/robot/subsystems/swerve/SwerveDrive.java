...
public class SwerveDrive extends SubsystemBase implements SwerveDriveInterface {
  ...
  public DriveInputsAutoLogged getInputs() { return inputs; }

  private SwerveDrive() {
    SwerveModule[] modules = {
        new SwerveModule(SwerveConstants.DFLPORT, SwerveConstants.AFLPORT,
                         SwerveConstants.FRONT_LEFT_MODULE_TRANSLATION, true,
                         false, 0.1, 0.3, false, false),
        new SwerveModule(SwerveConstants.DFRPORT, SwerveConstants.AFRPORT,
                         SwerveConstants.FRONT_RIGHT_MODULE_TRANSLATION, false,
                         false, 0.1, 0.2, false, false),
        new SwerveModule(SwerveConstants.DBLPORT, SwerveConstants.ABLPORT,
                         SwerveConstants.BACK_LEFT_MODULE_TRANSLATION, true,
                         false, 0.1, 0.3, false, false),
        new SwerveModule(SwerveConstants.DBRPORT, SwerveConstants.ABRPORT,
                         SwerveConstants.BACK_RIGHT_MODULE_TRANSLATION, false,
                         false, 0.1, 0.3, false, false),
    };
  ...
  /**
   * Gets the instance of the swerve drive. If the instance doesn't exist, it
   * creates it
  ...
  /**
   * A method from Subsystem Base. <p>Our code probably shouldn't call this.
   * This is called every scheduler tick (20ms) by WPI. Code that needs to be
   * run repeadly should be put here
   */
  @Override
  ...
  /**
   * Drives the robot relative to the field (forward is away from the alliance
   * wall) based on target forward, sideways, and rotational velocities
   * @param forwardVelocity the target forward velocity of the robot. Units are
   *     m/s and forward is positive
   * @param sidewaysVelocity the target sideways velocity of the robot. Units
   *     are m/s and left is positive
   * @param rotationalVelocity the target rotational velocity of the robot.
   *     Units are rad/s and counter-clockwise is positive
   */
  public void driveFieldCentric(double forwardVelocity, double sidewaysVelocity,
                                double rotationalVelocity) {
    driveModules(ChassisSpeeds.fromFieldRelativeSpeeds(
        forwardVelocity, sidewaysVelocity, rotationalVelocity,
        new Rotation2d(getRobotAngle())));
  }

  /**
   * Drives the robot relative to itself based on target forward, sideways, and
   ...
   *     Units are rad/s and counter-clockwise is positive
   */
  public void driveRobotCentric(double forwardVelocity, double sidewaysVelocity,
                                double rotationalVelocity) {
    driveModules(new ChassisSpeeds(forwardVelocity, sidewaysVelocity,
                                   rotationalVelocity));
  }

  /**
   * Uses inverse kinematics to convert a robot-relative set of chassis speeds
   * into states to send to the swerve modules
  ...
  public void driveModules(ChassisSpeeds targetChassisSpeeds) {
    SwerveModuleState[] states =
        kinematics.toSwerveModuleStates(discretize(targetChassisSpeeds)
                                        // targetChassisSpeeds
        );
    driveModules(states);
  }
  ...
  public void driveModules(SwerveModuleState[] states) {
    SwerveDriveKinematics.desaturateWheelSpeeds(
        states, SwerveConstants.MAX_LINEAR_SPEED);
  ...
  private static ChassisSpeeds discretize(ChassisSpeeds originalChassisSpeeds) {
    ...
    double dt = 0.02; // This should be the time these values will be used, so
                      // normally just the loop time
    Pose2d desiredDeltaPose =
        new Pose2d(vx * dt, vy * dt, new Rotation2d(omega * dt));
  ...
  /**
   * Uses forward kinematics to turn the swerve module's states into a chassis
   ...
   */
  public Pose2d getRobotPose() { return odometry.getPoseMeters(); }
   ...
   */
  public double getRobotAngle() { return gyro.getAngle() - currentGyroZero; }
   ...
   */
  public void resetRobotAngle() { resetRobotAngle(0); }

  public double getCurrentZero() { return currentGyroZero; }
  ...
  /**
   * A method from SubsystemBase. <p>Adds properties of the subsystem to
   * shuffleboard. Our code should never call this class, that is done by WPI
   */
  @Override
  public void initSendable(SendableBuilder builder) {
    builder.addDoubleProperty("Gyro Angle: ", () -> gyro.getAngle(), null);
    builder.addDoubleProperty(
        "Gyro Offset From Zero: ", () -> getRobotAngle() % (2 * Math.PI), null);
    builder.addDoubleProperty("Current Forward Speed: ",
                              () -> getChassisSpeeds().vxMetersPerSecond, null);
    builder.addDoubleProperty("Current Sideways Speed: ",
                              () -> getChassisSpeeds().vyMetersPerSecond, null);
    builder.addDoubleProperty(
        "Current Rotational Speed: ",
        () -> getChassisSpeeds().omegaRadiansPerSecond, null);
    builder.addDoubleProperty(
        "Odometric X: ", () -> getRobotPose().getX(), null);
    builder.addDoubleProperty(
        "Odometric Y: ", () -> getRobotPose().getY(), null);
    builder.addDoubleProperty(
        "Odometric Rotation: ",
        () -> getRobotPose().getRotation().getRadians(), null);
