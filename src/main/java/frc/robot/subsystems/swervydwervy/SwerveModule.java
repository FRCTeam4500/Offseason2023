...
public class SwerveModule {
	...
	public SwerveModule(
		TalonMotorController driveMotor,
		TalonMotorController angleMotor,
	...
	public void setDesiredState(
		...
		if (isDriveOpenLoop) {
			setDriveOpenLoopMetersPerSecond(
	...
	private SwerveModuleState setAngleOptimizedState(
		SwerveModuleState desiredState
	) {
		...
		double countsFromAngle =
			...
			(2.0 * Math.PI) *
			angleCountsPerRev;
		double countsDelta = Math.IEEEremainder(
			countsFromAngle - countsBefore,
			angleCountsPerRev
		);
	...
	private double getDriveMetersPerSecond() {
		...
		double motorRotationsPer100ms =
			encoderCountsPer100ms / driveCountsPerRev;
		double wheelRotationsPer100ms = motorRotationsPer100ms * driveGearRatio;
		double metersPer100ms =
			wheelRotationsPer100ms * wheelCircumferenceMeters;
	...
	private double getDrivePositionMeters() {
		...
		double wheelPositionMeters =
			wheelPositionTicks * wheelCircumferenceMeters;
	...
	private void setDriveClosedLoopMetersPerSecond(double metersPerSecond) { // Gets the robot going at specified
		...
		double wheelRotationsPerSecond =
			metersPerSecond / wheelCircumferenceMeters;
		double motorRotationsPerSecond =
			wheelRotationsPerSecond / driveGearRatio;
		double encoderCountsPerSecond =
			motorRotationsPerSecond * driveCountsPerRev;
		driveMotor.set(
			ControlMode.Velocity,
			encoderCountsPerSecond / k100msPerSecond
		);
	}
