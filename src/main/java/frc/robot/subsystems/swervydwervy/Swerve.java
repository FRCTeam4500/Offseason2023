...
public class Swerve extends SubsystemBase implements SwerveInterface {
    ...
    private OdematicsStrat odometry;
    ...
    private Swerve(Gyro gyro, SwerveModule... swerveModules) {
        ...
        SwerveModulePosition[] modulePositions = Arrays.stream(swerveModules)
                .map(SwerveModule::getPosition)
                .toArray(SwerveModulePosition[]::new);

        kinematics = new SwerveDriveKinematics(translation2ds);
        odometry = new OdematicsStrat(
    ...
    public static synchronized Swerve getInstance() {
        if (instance == null) {
            instance = new Swerve(
                    new SwerveModule(
                            new TalonMotorController(SwerveConstants.DFLPORT, "Talon FX"),
                            new TalonMotorController(SwerveConstants.AFLPORT, "Talon FX"),
                            SwerveConstants.FRONT_LEFT_MODULE_TRANSLATION),
                    new SwerveModule(
                            new TalonMotorController(SwerveConstants.DFRPORT, "Talon FX"),
                            new TalonMotorController(SwerveConstants.AFRPORT, "Talon FX"),
                            SwerveConstants.FRONT_RIGHT_MODULE_TRANSLATION),
                    new SwerveModule(
                            new TalonMotorController(SwerveConstants.DBLPORT, "Talon FX"),
                            new TalonMotorController(SwerveConstants.ABLPORT, "Talon FX"),
                            SwerveConstants.BACK_LEFT_MODULE_TRANSLATION),
                    new SwerveModule(
                            new TalonMotorController(SwerveConstants.DBRPORT, "Talon FX"),
                            new TalonMotorController(SwerveConstants.ABRPORT, "Talon FX"),
    ...
    /**
     * Replace the default {@code OdematicsStrat} with another such as
     * {@code
     * EstimatorStrat}. This is provided as a setter so that the
     * current configured
     ...
     */
    public void setOdometry(OdematicsStrat odometry) {
